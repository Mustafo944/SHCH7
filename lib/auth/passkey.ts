...
export type PasskeyFailure = 'cancelled' | 'unsupported' | 'network' | 'device' | 'unknown'
...
export function classifyPasskeyError(err: unknown): PasskeyFailure {
  ...
  // Android Chrome'ning Credential Manager (Google Password Manager) xizmati
  // o'zi ichki nosozlik bilan rad etganda shunday keladi — WebAuthn nomlari
  // (NotAllowedError va h.k.) bilan emas, balki "NotReadableError" yoki
  // umuman nomsiz, matnida "credential manager" so'zi bilan. Bu ilova kodi
  // yoki serverdagi bug emas, qurilma/OS darajasidagi vaqtinchalik nosozlik —
  // shuning uchun alohida ajratib, foydalanuvchiga aniq maslahat beramiz.
  if (name === 'NotReadableError' || /credential manager/i.test(message)) return 'device'
