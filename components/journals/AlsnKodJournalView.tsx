...
export function AlsnKodJournalView({
      ...
      if (doc?.entries?.length) {
        ...
        if (filtered.length > 0) {
          ...
          setEntries(prev => {
            ...
            return Array.from({ length: 5 }, EMPTY_ALSN_KOD)
          })
        }
      } else {
        setAllEntries([])
  ...
  useRealtimeSubscription(
    stationId && journalMonth
      ? [
          {
            channelName: `journal_alsnKod_${userRole}_${stationId}_${journalMonth}`,
  ...
  const addRow = () => { if (!isWorker) return; const n = [...entries, { ...EMPTY_ALSN_KOD(), _isNew: true }]; setEntries(n); }
